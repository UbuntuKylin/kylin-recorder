...
bool ItemsWindow::eventFilter(QObject *obj, QEvent *event)   //鼠标滑块点击
{
    ...
    {
        itemsWid->setAttribute(Qt::WA_Hover,true);//**非常关键的代码,悬浮特效**
        ...
        {
            //qDebug()<<"进入";
            listNumChangeColor->setStyleSheet("font-size:14px;color:#F84A77;");
            recordFileNameChangeColor->setStyleSheet("font-size:14px;color:#F84A77;");
            int nIndex = stackLayout->currentIndex();
            nIndex++;
            stackLayout->setCurrentIndex(1);//切换至录音按钮stackLayout
            splitLinestackWid->setCurrentIndex(1);//2020.11.12隐藏此滑动条功能
        }
        else if(event->type() == QEvent::HoverLeave)//收起浮窗
        {
            //qDebug()<<"离开";
            //此处为鼠标悬停离开某item时,字体颜色变化要注意和主题对应黑主题白字白主题黑字
            if(MainWindow::mutual->theme == "ukui-default"||MainWindow::mutual->theme == "ukui-white")
            {
