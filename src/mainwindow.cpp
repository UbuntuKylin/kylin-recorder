    ...
    :QMainWindow(parent)
{
    ...
    setButton->setIcon(QIcon::fromTheme("application-menu"));//主题库的菜单图标
    miniButton = new QToolButton(this);
    miniButton->setIcon(QIcon(":/png/png/mini.png"));
...
//  titleLeftWid->setStyleSheet("background-color:pink;");//测试用法
    ...
    rightMainWidLayout->setContentsMargins(4,4,4,0);
...
//        itemswindow->setStyleSheet("background-color:#222222;");//后期适配主题颜s;
    ...
    {
        listWid->setStyleSheet("background-color:#FFFFFF;"
                               "border-top-right-radius:0px;"
                               "border-bottom-right-radius:0px;");//后期适配主题颜色
...
//        itemswindow->recordFileName->setStyleSheet("font-size:14px;color:black;");
        ...
        //主界面
        mainWid->setObjectName("mainWid");//设置命名空间
        mainWid->setStyleSheet("#mainWid{background-color:#FFFFFF;border:1px solid #262626;opacity:0.15;}");//自定义窗体(圆角+背景色)
        recordButtonWid->setStyleSheet("background-color:#E5E5E5;opacity:0.1;");
